...
void SudokuSolver::solveBoard(int startingIndex, std::vector<int> &board, 
	...
	if (board[startingIndex] == 0) {
		...
		for (int potentialValue: potentialValues) {
			...
			if (potentialValue != 0) {
				...
				solveBoard(startingIndex + 1, board, givenIndexes);
			} 
		}
